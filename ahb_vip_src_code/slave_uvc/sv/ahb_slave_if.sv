...
//--------------------------------------------------------------
*/
interface ahb_slave_if #(ADDR_WIDTH = 32, HBURST_WIDTH = 3, HPROT_WIDTH = 4, DATA_WIDTH = 32) (input bit HCLK);                                                                                              

	logic HRESETn;     // 1-bit active low reset signal                                                                                                                                                                                  
	logic HSELx;     // 1-bit select signal. From Decoder to Slave. There are multiple slave and this signal indicates that the current transfer is intended for the selected Subordinate                          
	logic [ADDR_WIDTH-1:0] HADDR;     // Address of transfer. From Master to Decoder and Slave                                                                                                    
	logic HWRITE;     // 1-bit signal which Indicate flow of transfer. From Master to Slave. Indicate WRITE transfer when HIGH and READ transfer when LOW                                                         
	logic [2:0] HSIZE;     // 3-bit signal from Master to Slave. Indicate size of transfer                                                                             
	logic [HBURST_WIDTH-1:0] HBURST;     // Indicates how many transfers are in the burst and how the address increments. HBURST_WIDTH must be 0 or 3 . From Master to Slave                    
	logic [HPROT_WIDTH-1:0] HPROT;     // Protection control Signal which give information about access type. From Master to Slave                                                      
	logic [1:0] HTRANS;     // 2-bit signal from Master to Slave. Indicates the transfer type ( IDLE, BUSY, NONSEQUENTIAL, SEQUENTIAL)                                                                                 
	logic HMASTLOCK;     // 1-bit signal from Master to Slave. Indicate that current transfer is part of locked sequence                                                                                
	logic HREADY;     // 1-bit signal from Multiplexor to Master and Slave. Indicate previous transfer is completed when HIGH                                                                          
	logic [DATA_WIDTH-1:0] HWDATA;     // Write Data value. From Master to Slave during WRITE operation                                                                                           
	logic HREADYOUT;     // 1-bit signal from Slave to Multiplexor. When HIGH indicate that transfer is completed on bus                                                                                
	...
	logic [DATA_WIDTH-1:0] HRDATA;     // Read Data value. From Slave to Multiplexor during Read operation                                                                          
	

//-----------------------------------------------------------------------------------------------------------------------------------------------
// Uncomment the following properties if  u want to mimic the RESP == 1 behavior of SLAVE DUT
//-----------------------------------------------------------------------------------------------------------------------------------------------
...
/*
error_behaviour:  assert property(@(posedge HCLK)
HRESP |=> HRESP
);


error_happening_transaction_termination:   assert property (@(posedge HCLK)
(HRESP&&(!HREADY)) ##1 (HRESP&&HREADY)|-> (HTRANS==0)
);

cover_error_behaviour:  cover property(@(posedge HCLK)
HRESP |=> HRESP
);


cover_error_happening_transaction_termination:   cover property (@(posedge HCLK)
(HRESP&&(!HREADY)) ##1 (HRESP&&HREADY)|-> (HTRANS==0)
);
...
//--------------------------------------------------------------------------------------------------------------------------------------
// Uncomment the following properties if  u want to use the UVC standalone with a SLAVE DUT
//--------------------------------------------------------------------------------------------------------------------------------------
